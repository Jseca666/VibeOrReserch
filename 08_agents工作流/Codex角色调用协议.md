...
本协议规定 T0-T7 如何调度；每轮任务如何从输入触发运行到 evolution report 和下一轮任务包，见 `自进化自动工作闭环协议.md`。

## 0. Auto-Gate 兼容说明

自 NXT-20260502-008 起，日常任务路由、产物验收、债务推进、暂缓、降级、关闭候选和下一轮任务生成，默认由 Auto-Gate 进行系统内保守裁决。历史文本中的 `PI导师`、`PI裁决`、`PI待裁决` 不表示用户需要亲自承担数学终审；在日常推进语境下按 Auto-Gate 系统内裁决解释，在投稿、公开发布、最终强 claim 表述和重大方向取舍前按人类责任确认解释。

`PI导师` 作为角色名保留，用于对外责任确认和重大方向确认；不再作为每轮普通研究任务的默认卡点。

## 1. 四层边界

| 层级 | 定位 | 边界 |
|---|---|---|
| Codex | 执行与推理主体 | 负责组织任务、生成草案、调用角色和技能，但不替代 Auto-Gate 门禁裁决或最终对外责任确认 |
| 项目角色系统 | 责任、门禁、审计和裁决依据层 | 负责签收、审查、阻断、建议，并向 Auto-Gate 提供系统内裁决依据 |
| EvoSkills | 能力工具箱 | 只增强检索、推导、证明、实验、写作和记忆能力；不签收、不裁决、不关闭债务 |
| EvoScientist/evo-memory | 长期记忆层 | 记录跨周期经验，不替代研究债务队列、claim 审批表、Auto-Gate 门禁裁决和最终对外责任确认 |
...
| 任务类型 | 名称 | 适用场景 | 默认角色组合 | 可调用 EvoSkills | 必须产物 | 门禁检查 | 研究债务记录 | Auto-Gate 裁决 / 人类确认条件 |
|---|---|---|---|---|---|---|---|---|
| T0 | 论文方向与贡献策略调度 | 新方向启动前、重技术 NXT 启动前、文献边界改变贡献定位时、技术产物准备进入 proof package / paper progress / slides 前、用户询问“这个方向能否成论文”时 | `论文边界与新颖性副导师`、`论文叙事与审稿策略顾问`、`Auto-Gate`、`研究债务协调员`；按主题加入 `Wasserstein-DRO副导师`、`运筹最优化副导师`、`实验与Benchmark副导师` 或 `风-能耗物理建模专家` | `paper-planning`、`paper-navigator`、`research-survey`、`research-lit`、`paper-review`、`evo-memory` | paper strategy note、related work map、contribution evidence matrix、审稿风险清单、后续技术 NXT 的论文锚点 | 必须说明论文主题、核心研究问题、候选贡献、证据缺口、related work 交叉点和禁止表述；不得把候选贡献写成已成立 claim | 论文主题不清、贡献候选无证据、related work 未定位、技术任务无论文锚点时，登记为 `待查文献`、`待补证据` 或生成 T0/NXT | Auto-Gate 可自动暂缓无锚点技术扩展；涉及重大方向投入、投稿或最终强 claim 时需要人类确认 |
...
| T6 | 论文 claim 升级 | 将命题、模型、实验结果或文献边界结论升级为论文 claim | `Auto-Gate`、相关副导师、`证明审计员`、`论文边界与新颖性副导师`、`研究债务协调员`；涉及投稿或公开强 claim 时加入 `PI导师` | `result-to-claim`、`paper-planning`、`paper-review`、`paper-write`、`paper-compile`、`novelty-check`、`research-review` | claim 升级建议、证据包链接、未关闭债务清单、Auto-Gate 裁决项与人类确认需求 | 存在未关闭 `claim阻断` 债务时不得升级强 claim；证明审计、文献边界、实验支撑必须与 claim 类型匹配 | 升级被阻断时记录阻断原因、负责人角色、关闭条件、Auto-Gate 状态和人类确认边界 | 先由 Auto-Gate 保守裁决；投稿、公开发布或强 claim 写入前需要人类责任确认 |
| T7 | 角色/主线自进化 | 角色职责不匹配、任务反复卡住、主线需要调整、需要新增或暂缓角色、需要整理跨周期经验 | `Auto-Gate`、`研究债务协调员`、相关副导师；涉及重大方向取舍时加入 `PI导师` | `evo-memory`、`research-review`、`paper-planning`、`mermaid-diagram` | 角色变更提案、影响 claim 清单、关联债务、Auto-Gate 裁决记录、记忆写回建议 | 角色变更必须落档；副导师、执行专家、研究债务协调员和 EvoSkills 不能自行改变角色结构 | 角色缺口、职责冲突、反复失败模式登记为 `待更新角色` 或写入研究记忆 | Auto-Gate 预审并给出系统内裁决；重大方向改变或对外责任事项需要人类确认 |

补充边界：

- T3 中的 `proof-writer` 仅在涉及模型等价、有效不等式、reformulation 正确性或离散化精确性证明时调用；普通模型卡、变量表和约束表不强制调用。
- T4 中的 `result-to-claim` 只能生成候选 claim 线索；任何论文 claim 升级必须转入 T6，并由 Auto-Gate 保守裁决；投稿、公开发布或强 claim 写入前需要人类责任确认。
...
|---|---|---|
| `PI导师` | `research-review`、`paper-planning`、`result-to-claim` | 对外责任确认和重大方向确认不能由技能替代 |
...
- 技能不替代 Auto-Gate 与人类确认：任何 claim 升级、主线改变、角色新增或弱化，必须进入 Auto-Gate 系统内裁决；投稿、公开发布、最终强 claim 表述和重大方向取舍前需要人类责任确认。
...
所有未来 claim，例如 C-020、新 DRCC 命题、新实验任务、新文献边界任务，均按 T0-T7 进入。
